...
async function resetPassword(email) {
  ...
  try {
    ...
    const res = await pool.query('UPDATE employees SET password_hash = $1 WHERE email = $2 RETURNING id, email', [hashed, email]);
    if (res.rowCount === 0) {
...
if (!email) {
  console.error('Usage: node resetPasswordByEmail.js <email>');
  process.exit(1);
}
